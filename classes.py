...
class World:
	...
	def can_move(self, from_location, direction): # Проверка возможности перемещения
		location = self.get_location(from_location)
		return direction in location["paths"]

	def move(self, from_location, direction): # Возвращает ID новой локи, в которую пришел герой
		if self.can_move(from_location, direction): return location["paths"][direction]
		return None # непонятно к чему return
...
class Player:
	...
	def attack(self): # Базовая атака
		if self.weapon:
			return self.weapon.calculate_damage()
		else:
			return {"damage": 3, 
			"is_crit": False, 
